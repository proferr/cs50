...
bool vote(string name)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // If candidate is similar to what is user's vote
    ...
    // If not similar to candidate vote (print invalid vote)
    return false;
}
...
void print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // Candidate votes greater than maximum_vote - set maximum_vote
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // Candidate votes equal to maximum vote - print it
        if (candidates[i].votes == maximum_vote)
        {
            printf("%s\n", candidates[i].name);
        }
